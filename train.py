...
save_path=os.path.join('./ckpt/', model_name)
min_mae = 1
validation_step = 0
...
logfile = os.path.join(save_path, model_name+'_log.txt')
open(logfile, 'a').write('\n'+'Start Train: '+str(datetime.now())+ '\n')
global_step = 0
for epoch in range(1, opt.epoch + 1):
    generator.train()
    loss_record = AvgMeter()
    print('Generator Learning Rate: {}'.format(generator_optimizer.param_groups[0]['lr']))
    open(logfile, 'a').write('Generator Learning Rate: {}'.format(generator_optimizer.param_groups[0]['lr'])+ '\n')
    ...
    for i, pack in enumerate(train_loader, start=1):
        ...
        log='Epoch[{:03d}/{:03d}],Step[{:04d}/{:04d}],gen Loss:{:.4f}'.format(epoch, opt.epoch, i, total_step, loss_record.show())
        train_loader.set_description(log)

        if i % 100 == 0 or i == total_step or i == 1:
            open(logfile, 'a').write('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], gen Loss: {:.4f}\n'.
