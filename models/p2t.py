...
class IRB(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, ksize=3, act_layer=nn.Hardswish, drop=0.):
        ...
        self.conv = nn.Conv2d(hidden_features, hidden_features, kernel_size=ksize, padding=ksize // 2, stride=1,
                              groups=hidden_features) # 3*3采样，groups=hidden_features深度可分离卷积
    ...
    def forward(self, x, H, W):
        ...
        x = x.permute(0, 2, 1).reshape(B, C, H, W) # B C N(H*W) -> B C H W
        # 1*1扩充通道
        x = self.fc1(x) #
        # hardwish 激活函数
        x = self.act(x)
        ...
        x = self.fc2(x)
        # 变回1维
        return x.reshape(B, C, -1).permute(0, 2, 1) # B C N -> B N C
