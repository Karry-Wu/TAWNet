...
sys.path.append('..')
results_save_path = './pred/'
torch.manual_seed(118)

model_name = 'TAWNet'
ckpt_path=os.path.join('./ckpt/', model_name)
parser = argparse.ArgumentParser()
parser.add_argument('--ss', default='TAWNet', type=str, help='snapshot')  #用哪个模型，字符串类型，不加.pth
...
def main():
    ...
    with torch.no_grad():
        for name, root in to_test.items():
            print(name, root)
            root1 = os.path.join(root, data_modal)  # depth root
            img_list = [os.path.splitext(f) for f in os.listdir(root1)]  #[('image1', '.jpg'), ('image2', '.png')]
            # print(img_list)
            for idx, img_name in enumerate(img_list):
                print('%s predicting for %s: %d / %d' % (opt.ss, name, idx + 1, len(img_list)))
                ...
                if os.path.exists(modal_jpg_path):
                    modality = Image.open(modal_jpg_path)
                else:
                    modality = Image.open(modal_png_path)

                if data_modal == 'depth':
                    modality = modality.convert('L')  #用于将图像转换为灰度图像
                else:
                ...
                if data_modal == 'depth':
                    modality = torch.cat((modality_var, modality_var, modality_var), 1)  # depth*3
                else:
                ...
                prediction = prediction.resize((w_, h_), Image.BILINEAR)  # upsamlple

                if args['save_results']:
                    check_mkdir(os.path.join(results_save_path, model_name, args['snapshot'], name))
                    prediction.save(os.path.join(results_save_path, model_name, args['snapshot'], name, img_name[0] + '.png'))
